...
def create_lesson_task(self, lesson_id: int = None):
    ...
    log.info(
        "Длина списка студентов %s, к уроку: %s",
        # "Длина списка студентов %s, к уроку: %s c task_id: %s",
        len(students),
        lesson_id,
    ...
    for student in students:
        log.warning(f" c ID[{student.id}] по уроку {lesson.title}")
        time.sleep(3)  # Задержка для симуляции отправки
